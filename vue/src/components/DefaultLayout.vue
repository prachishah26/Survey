        ...
        <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
            ...
            <DisclosurePanel class="md:hidden">
                <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
                    <router-link v-for="item in navigation" :key="item.name" :to="item.to" :href="item.href"
                        :class="[item.$route.name === item.to.name ? '' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium']"
                        active-class="bg-gray-900 text-white">{{ item.name }}</router-link>
                </div>
...
//   import { Bars3Icon, BellIcon, XMarkIcon } from '@heroicons/vue/solid'
